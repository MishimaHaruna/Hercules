...
char* HPM_file2ptr(const char *file) {
	...
	if( HPM->fnames == NULL ) {
		HPM->fnames = malloc(sizeof(struct HPMFileNameCache));
	} else {
		void *p = malloc(sizeof(struct HPMFileNameCache)*HPM->fnamec);
		memcpy(p,HPM->fnames,sizeof(struct HPMFileNameCache)*(HPM->fnamec-1));
		free(HPM->fnames);
		HPM->fnames = p;
	}
