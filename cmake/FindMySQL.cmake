...
elseif(MYSQL_CONFIG_EXECUTABLE)
  ...
  foreach(_xvar ${MYSQL_INCLUDE_DIRS})
    if(EXISTS "${_xvar}/mysql.h")
      set(MYSQL_INCLUDE_DIR ${_xvar})
      break()
    endif()
  endforeach()

  if(NOT MYSQL_INCLUDE_DIR)
    message(FATAL_ERROR "Could not find \"mysql.h\" in \"${MYSQL_INCLUDE_DIRS}\" "
...
elseif(MYSQL_CONFIG_EXECUTABLE)
  ...
  _mysql_config(MYSQL_LIB_DIRS "(^| )-L" "--libs")
  if(NOT MYSQL_LIB_DIRS)
    message(FATAL_ERROR "Could not find the library dir from running "
                        "\"${MYSQL_CONFIG_EXECUTABLE}\"")
  endif()

  foreach(_xvar ${MYSQL_LIB_DIRS})
    if(EXISTS "${_xvar}")
      set(MYSQL_LIB_DIR ${_xvar})
      break()
    endif()
  endforeach()

  if(NOT MYSQL_LIB_DIR)
    message(FATAL_ERROR "Could not find a valid directory in \"${MYSQL_LIB_DIRS}\" "
                        "found from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
  endif()
